...
_viewJs.UI.MESSANGER.BUTTON_SEND.addEventListener("click", (e)=>{
    e.preventDefault();
    const currentMassage = _viewJs.UI.MESSANGER.INPUT_SEND.value;
    if (!currentMassage) return;
    createTemplate(currentMassage, _viewJs.UI.MESSANGER.TEMPLATE);
    _viewJs.UI.MESSANGER.FORM.reset();
});
_viewJs.UI.SETTINGS.BUTTON.addEventListener('click', (e)=>{
    e.preventDefault();
    sendRequest('PATCH', `${_viewJs.URL}/api/user`, {
        name: _viewJs.UI.SETTINGS.INPUT.value
    });
    _viewJs.UI.SETTINGS.FORM.reset();
});
_viewJs.UI.AUTHORIZATION.BUTTON.addEventListener("click", (e)=>{
    e.preventDefault();
    sendRequest('POST', `${_viewJs.URL}/api/user`, {
        email: _viewJs.UI.AUTHORIZATION.INPUT.value
    });
    _viewJs.UI.AUTHORIZATION.FORM.reset();
});
_viewJs.UI.CONFIRM.BUTTON.addEventListener('click', (e)=>{
    e.preventDefault();
    _jsCookieDefault.default.set('token', _viewJs.UI.CONFIRM.INPUT.value);
    _viewJs.UI.CONFIRM.FORM.reset();
});
function getDate() {
    const date = new Date();
...
function createTemplate(message, template) {
    ...
    _viewJs.UI.MESSANGER.SCREEN.append(clone);
}
...
},{"./view.js":"de63B","js-cookie":"c8bBu","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"de63B":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
